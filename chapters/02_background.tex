...
% TODO: Those eqs are not correct yet
\begin{equation}%
...
Finding the weights $a_ij$ and $b_ij$ can be done by minimizing the square
...
activations map the latent vectors to archetype coefficients, the $k$ archetype
vectors are initialized to be the vertices of a $(k-1)$-simplex, as there is no
absolute frame of reference for the latent space coordinates. The archetype
